...
class registerScreenViewController: UIViewController {
   ...
   var isAgree = false
    ...
    @IBAction func addUser(_ sender: UIButton) {
        let item = User(context: context1)
        if let name = txtName.text , txtName.text != "" {
    
            item.name = name
        
        if let email = txtEmail.text , txtEmail.text != ""
        {
            item.email = email
        
        if let pass = txtPass.text , txtPass.text != ""
        {
            if  pass == txtPassConfirm.text
            {
            item.password = pass
                do {
                ...
                let alert = UIAlertController(title: "Alert", message: "Confirm Password is wrong ", preferredStyle: .alert)
                let action = UIAlertAction(title: "OK", style: .cancel, handler: {
                    action in
                    print("tapped")
                })
            ...
            let alert = UIAlertController(title: "Alert", message: "Password is required ", preferredStyle: .alert)
            let action = UIAlertAction(title: "OK", style: .cancel, handler: {
                action in
                print("tapped")
            })
            ...
            let alert = UIAlertController(title: "Alert", message: "Email is required ", preferredStyle: .alert)
            let action = UIAlertAction(title: "OK", style: .cancel, handler: {
                action in
                print("tapped")
            })
            ...
            self.present(alert, animated: true, completion: nil)
                ...
                let alert = UIAlertController(title: "Alert", message: "User Name is required ", preferredStyle: .alert)
                let action = UIAlertAction(title: "OK", style: .cancel, handler: {
                    action in
                    print("tapped")
                })
